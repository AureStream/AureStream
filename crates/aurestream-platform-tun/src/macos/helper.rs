...
mod ffi {
    ...
    extern "C" {
        ...
        pub fn aurestream_helper_flush_dns_cache(error_out: *mut *mut c_char) -> c_int;
        pub fn aurestream_helper_remove_tun_routes(
            interface_name: *const c_char,
            error_out: *mut *mut c_char,
        ) -> c_int;

        pub fn aurestream_helper_set_exit_callback(cb: ExitCallback);
        pub fn aurestream_helper_free_string(s: *mut c_char);
    }
}
...
fn consume_cstring(ptr: *mut std::os::raw::c_char) -> String {
    ...
    unsafe { ffi::aurestream_helper_free_string(ptr) };
    s
}
...
pub mod api {
    ...
    pub fn ping() -> Result<String, String> {
        ...
        let rc = unsafe { ffi::aurestream_helper_ping(&mut reply) };
    ...
    pub fn set_dns_servers(service_name: &str, dns_spec: &str) -> Result<(), String> {
        ...
        call_error_only(|err_out| unsafe {
            ffi::aurestream_helper_set_dns_servers(c_service.as_ptr(), c_spec.as_ptr(), err_out)
        })
    }

    pub fn flush_dns_cache() -> Result<(), String> {
        call_error_only(|err_out| unsafe { ffi::aurestream_helper_flush_dns_cache(err_out) })
    }

    pub fn remove_tun_routes(interface_name: &str) -> Result<(), String> {
        let c_iface = to_cstring(interface_name, "interface_name")?;
        call_error_only(|err_out| unsafe {
            ffi::aurestream_helper_remove_tun_routes(c_iface.as_ptr(), err_out)
        })
    }
}
