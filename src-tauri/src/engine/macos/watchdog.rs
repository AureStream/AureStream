    ...
    loop {
        ...
        let still_tun = {
            ...
            manager
                .mode
                .as_ref()
                .map(|m| **m == ProxyMode::IntoProxy) // In AureStream, TunProxy is called IntoProxy
                .unwrap_or(false)
        };
        if !still_tun {
...
async fn restart_tun_send_safe(app: AppHandle, path: Arc<String>) -> Result<(), String> {
...

    {
        ...
        manager.mode = Some(Arc::new(ProxyMode::IntoProxy)); // In AureStream, TunProxy is IntoProxy
