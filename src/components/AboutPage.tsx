...
export default function AboutPage() {
  ...
  const features = [
    {
      ...
      desc: l("Military-grade encryption protects all traffic", "军用级加密保护全部流量"),
    },
    {
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      ...
      desc: l("Rule-based routing for optimal experience", "基于规则的分流策略，体验更佳"),
    },
    {
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      ...
      desc: l("sing-box core for blazing fast connections", "sing-box 内核，极速稳定连接"),
    },
    {
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      ...
      desc: l("macOS, Windows and Linux supported", "支持 macOS、Windows 和 Linux"),
    },
  ]
  ...
  const techStack = [
    ...
    { label: "Core Engine", value: `sing-box ${SING_BOX_VERSION}` },
    { label: "Protocols", value: "VLESS · VMess · Trojan" },
  ]
    ...
    <div className="flex flex-col w-full h-full max-w-[420px] mx-auto animate-fade-in">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 pb-6 flex flex-col gap-4">
        ...
        <div className="bg-white dark:bg-bg-alt rounded-3xl p-5 shadow-sm border border-slate-100 dark:border-white/10 flex flex-col items-center gap-3">
          <div className="w-20 h-20 rounded-[24px] bg-gradient-to-br from-[#00BBA7] to-[#0094A0] flex items-center justify-center shadow-lg">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 font-semibold mt-0.5">{osType}</p>
          </div>
          ...
          <p className="text-center text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed px-2">
            {l(
        ...
        <div className="bg-white dark:bg-bg-alt rounded-3xl p-4 shadow-sm border border-slate-100 dark:border-white/10">
          <h2 className="text-sm font-black text-slate-700 dark:text-slate-200 mb-3 px-1">{l("Features", "核心功能")}</h2>
          <div className="grid grid-cols-2 gap-2.5">
            {features.map((f, i) => (
              <div key={i} className="bg-slate-50 dark:bg-white/5 rounded-2xl p-3.5 flex flex-col gap-2 border border-slate-100 dark:border-white/5">
                <div className="w-9 h-9 rounded-xl bg-[#00BBA7]/10 text-[#00BBA7] flex items-center justify-center">
                  {f.icon}
                </div>
                <div>
                  <div className="text-sm font-black text-slate-800 dark:text-white leading-tight">{f.title}</div>
                  <div className="text-[11px] text-slate-400 font-medium mt-0.5 leading-relaxed">{f.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Tech Stack */}
        <div className="bg-white dark:bg-bg-alt rounded-3xl p-4 shadow-sm border border-slate-100 dark:border-white/10">
          <h2 className="text-sm font-black text-slate-700 dark:text-slate-200 mb-3 px-1">{l("Tech Stack", "技术栈")}</h2>
          <div className="flex flex-col divide-y divide-slate-100 dark:divide-white/5">
            {techStack.map((item, i) => (
              <div key={i} className="flex items-center justify-between py-2.5 first:pt-0 last:pb-0">
                <span className="text-xs font-bold text-slate-400">{item.label}</span>
                <span className="text-xs font-black text-slate-700 dark:text-slate-200 font-mono">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-bg-alt rounded-3xl p-4 shadow-sm border border-slate-100 dark:border-white/10 flex flex-col gap-2">
          <h2 className="text-sm font-black text-slate-700 dark:text-slate-200 mb-1 px-1">{l("Links", "相关链接")}</h2>
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-slate-800 dark:bg-white/10 flex items-center justify-center shrink-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-slate-400 font-medium truncate">BadKid90s/AureStream</div>
            </div>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-slate-400 font-medium">sing-box.sagernet.org</div>
            </div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-slate-300 shrink-0">
              <polyline points="9 18 15 12 9 6"/>
            </svg>
        ...
        <div className="flex flex-col items-center gap-1.5 py-2">
          <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
            ...
            {l("Made with love by BadKid90s", "由 BadKid90s 用心制作")}
          </div>
          <p className="text-[10px] text-slate-300 dark:text-slate-600 font-medium">
            {l("Open source · MIT License", "开源软件 · MIT 许可证")}
          </p>
        </div>
