...
## 3. Command 入口 (`src-tauri/src/core/`)

### commands.rs - 连接生命周期命令
...
### 平台特异性实现
- **Windows**: `engine/windows/` 编排 SCM 服务 `AureStreamTunService`；WinINet 系统代理；UWP 环回豁免。
- **macOS**: `engine/macos/` 编排 SMJobBless + XPC Helper；`watchdog`；Helper 支持自卸载。
- **Linux**: `pkexec` + `aurestream-tun-helper`（源文件位于 `crates/aurestream-plugin-privilege/linux-helper/`，由 deb/rpm 安装到系统路径）；`systemd-resolved` DNS；deb/rpm 安装 polkit 策略与 udev 规则。
...
| 路径 | 说明 |
|------|------|
| `crates/aurestream-plugin-privilege/linux-helper/` | Linux pkexec helper、polkit policy/rules |
| `crates/aurestream-plugin-privilege/macos-helper/` | macOS SMJobBless privileged helper 源码与 plist |
| `crates/aurestream-plugin-privilege/src/windows.rs` | Windows UAC 提权入口 |
| `crates/aurestream-plugin-tun/` | Windows TUN SCM 服务与跨平台 TUN 业务逻辑 |
| `crates/aurestream-plugin-proxy/` | 跨平台系统代理设置 |
| `src-tauri/resources/linux/` | Linux deb/rpm 安装/卸载脚本 |
| `src-tauri/resources/main.desktop` | Linux 桌面入口 |
| `src-tauri/tauri.conf.json` | Tauri 打包资源映射 |
